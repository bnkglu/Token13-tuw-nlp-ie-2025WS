...
def create_answer_key(processed_data: list[dict], output_file: Path) -> None:
...
def save_results(train_report: str, test_report: str, output_file: Path) -> None:
...
def save_all_outputs(
    ...
    if rules:
        ...
        with open(rules_tsv, "w") as f:
            f.write("relation\tpattern_type\tprecision\tsupport\tpriority\tpattern_data\n")
            for rule in rules:
                f.write(
                    f"{rule['relation']}\t{rule['pattern_type']}\t"
                    f"{rule['precision']:.3f}\t{rule['support']}\t"
                    f"{rule.get('priority', 0):.0f}\t{rule['pattern_data']}\n"
                )
